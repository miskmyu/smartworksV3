...
//TODO webservice model 정의 필요
		...
		SwcWebService serviceObj = SwManagerFactory.getInstance().getSwcManager().getWebService(user, taskDef.getServiceTargetId(), IManager.LEVEL_ALL);
		if (serviceObj == null)
		...
		SwcWebServiceParameter[] webWebServiceParameter =  serviceObj.getSwcWebServiceParameters();
		String returnType = null;
		for (int i = 0; i < webWebServiceParameter.length; i++) {
			SwcWebServiceParameter wsp = webWebServiceParameter[i];
