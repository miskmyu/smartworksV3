...
public class MailContent extends MisObject {
	...
	public static BaseObject toObject(Node node, BaseObject baseObj) throws Exception {
		...
		MailContent obj = null;
		if (baseObj == null || !(baseObj instanceof MailContent))
			obj = new MailContent();
		else
			obj = (MailContent)baseObj;
	...
	public static MailContent[] add(MailContent[] objs, MailContent obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
			size = objs.length;
		MailContent[] newObjs = new MailContent[size+1];
		int i;
		for (i=0; i<size; i++)
			newObjs[i] = objs[i];
		return newObjs;
	...
	public static MailContent[] remove(MailContent[] objs, MailContent obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
	...
	public static MailContent[] left(MailContent[] objs, MailContent obj) {
		...
		if (idx < 1)
			return objs;
		MailContent[] newObjs = new MailContent[objs.length];
		for (int i=0; i<objs.length; i++) {
			if (i == idx) {
	...
	public static MailContent[] right(MailContent[] objs, MailContent obj) {
		if (objs == null || objs.length == 0 || obj == null)
			return objs;
		int idx = -1;
		for (int i=0; i<objs.length; i++) {
