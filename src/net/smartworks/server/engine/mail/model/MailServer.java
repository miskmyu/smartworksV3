...
public class MailServer extends MisObject {

	private static final long serialVersionUID = 1L;
	private static Log logger = LogFactory.getLog(MailServer.class);

	protected static final String PREFIX = "Mail";
	private static final String NAME = CommonUtil.toName(MailServer.class, PREFIX);
	...
	public static BaseObject toObject(Node node, BaseObject baseObj) throws Exception {
		...
		MailServer obj = null;
		if (baseObj == null || !(baseObj instanceof MailServer))
			obj = new MailServer();
		else
			obj = (MailServer)baseObj;
	...
	public static MailServer[] add(MailServer[] objs, MailServer obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
			size = objs.length;
		MailServer[] newObjs = new MailServer[size+1];
		int i;
		for (i=0; i<size; i++)
			newObjs[i] = objs[i];
		return newObjs;
	...
	public static MailServer[] remove(MailServer[] objs, MailServer obj) {
		if (obj == null)
			return objs;
		int size = 0;
		if (objs != null)
	...
	public static MailServer[] left(MailServer[] objs, MailServer obj) {
		...
		if (idx < 1)
			return objs;
		MailServer[] newObjs = new MailServer[objs.length];
		for (int i=0; i<objs.length; i++) {
			if (i == idx) {
	...
	public static MailServer[] right(MailServer[] objs, MailServer obj) {
		if (objs == null || objs.length == 0 || obj == null)
			return objs;
		int idx = -1;
		for (int i=0; i<objs.length; i++) {
