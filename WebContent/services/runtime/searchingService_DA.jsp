...
<%@page import="net.smartworks.server.engine.invoker.model.Invoker"%>
...
<%!
	//사용자
	public static String convertUserId(String xml, String userLanguage) throws Exception {

		String returnColumnName = "username";
		String functionName = "dbo.fn_getusername";
		Invoker invoker = new Invoker();
		invoker.setFunction(true);
		invoker.setFunctionName(functionName);
		invoker.setReturnColumn(new String[]{returnColumnName});
			...
			for (int i=0; i<actNodes.getLength(); i++) {
				...
				if (actElement != null) {
					...
					if (!CommonUtil.isEmpty(performerName)) {
						...
						invoker.setFunctionArgs(new String[]{performerName, userLanguage});
						Map resultMap = SwManagerFactory.getInstance().getInvokerManager().invoke(invoker);
						if (resultMap == null || CommonUtil.isEmpty(resultMap.get(returnColumnName))) {
							actElement.setAttribute("PerformerName", performerName);
						} else {
							actElement.setAttribute("PerformerName", (String)resultMap.get(returnColumnName));
						}
					}
				}
			}
...
<%
	...
	String[] userInfos = StringUtils.tokenizeToStringArray(userId, "|");
	userId = userInfos[0];
	String userLanguage = userInfos[1];
	...
	try {
		...
		// 사용자 이름 검색
			...
			xml = convertUserId(xml, userLanguage);
