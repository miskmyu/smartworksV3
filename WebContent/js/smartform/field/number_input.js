...
SmartWorks.FormRuntime.NumberInputBuilder.build = function(config) {
	...
	if(readOnly){
		if(value=='')
			...
			$number = $('<div class="form_value form_number_input" style="width:' + valueWidth + '%"><span></span></div>').find('span').text(value).formatCurrency({ symbol: '' ,colorize: true, negativeFormat: '-%s%n', roundToDecimalPlace: -1, eventOnDecimalsEntered: true });
	}else{	
