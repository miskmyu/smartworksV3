...
<script type="text/javascript">
	getIntanceList = function(paramsJson, progressSpan, isGray){
		...
		$.ajax({
			...
			success : function(data, status, jqXHR) {
				$('#department_instance_list_page').html(data);
	...
	selectListParam = function(progressSpan, isGray){
		var departmentList = $('.js_department_list_page');
		var forms = departmentList.find('form:visible');
		var paramsJson = {};
		paramsJson["href"] = "jsp/content/community/department_instance_list.jsp";
		var searchFilters = departmentList.find('form[name="frmSearchFilter"]');
		...
		if(isEmpty(progressSpan)) progressSpan = departmentList.find('.js_search_filter_page').next('span.js_progress_span:first');
...
<%
	...
	session.setAttribute("lastLocation", "department_list.sw");
	...
	if (params == null){
		...
		if(!SmartUtil.isBlankObject(savedWorkId) && savedWorkId.equals(Work.ID_DEPARTMENT_WORK)){
...
<!-- 컨텐츠 레이아웃-->
<div class="section_portlet js_department_list_page js_work_list_page">
		...
		<ul class="portlet_r" style="display: block;">
			...
			<div class="body_titl space">
				...
				<div class="noti_in_bodytitle" style="margin-top:25px">
					<div class="pr10 fl">
						<span>
							<a href="user_list.sw" class="js_content"><fmt:message key="common.title.user_communities"/></a>
						</span> | <span>
							<a href="group_list.sw" class="js_content"><fmt:message key="common.title.group_communities"/></a>
						</span> | <span class="title current"><fmt:message key="common.title.depart_communities"/></span>
					</div>
				</div>				
				<!-- 우측 버튼 -->
			...
			<div class="contents_space">
				...
				<div>
					...
					<div class="list_title_space js_work_list_title mt15">
						<div class="title"><fmt:message key="common.title.depart_list" /></div>					
							...
							<form class="form_space po_left js_form_filter_name" name="frmIworkFilterName">
								<select name="selFilterName" class="js_select_search_filter">
									<option value="<%=SearchFilter.FILTER_ALL_INSTANCES%>" 
										...
										<fmt:message key='filter.name.all_departs' />
									</option>
									<option value="<%=SearchFilter.FILTER_MY_ASCEND_DEPARTMENTS%>"
										<%if(SearchFilter.FILTER_MY_ASCEND_DEPARTMENTS.equals(selectedFilterId)){%> selected <%} %>>
										<fmt:message key='filter.name.my_ascend_departs' />
									</option>
									<option value="<%=SearchFilter.FILTER_MY_DESCEND_DEPARTMENTS%>"
										<%if(SearchFilter.FILTER_MY_DESCEND_DEPARTMENTS.equals(selectedFilterId)){%> selected <%} %>>
										<fmt:message key='filter.name.my_descend_departs' />
									</option>
								</select>
							</form>
					...
					<div class="list_contents">
						<div id='department_instance_list_page' >
 							<jsp:include page="/jsp/content/community/department_instance_list.jsp"/>
						</div>
					</div>
					<!-- 목록 테이블 //-->
				</div>
