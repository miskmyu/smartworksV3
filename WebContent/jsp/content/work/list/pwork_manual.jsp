...
<div class="contents_space js_pwork_manual_page js_sub_instance_list js_space_sub_instance" workId="<%=work.getId()%>" workType="<%=work.getType()%>">
	...
	<div class="border">
		...
		<form name="frmPWorkManual">	
			...
			<!-- 프로세스 영역 -->
			<div class="define_space" style="height:88px">
	        ...
	        <div class="list_reply">
	            <ul class="js_comment_list">
	            	<li class="js_comment_instance" style="display:none">
	            		<div class="det_title">
							<div class="noti_pic">
								<a class="js_pop_user_info" href="<%=cUser.getSpaceController() %>?cid=<%=cUser.getSpaceContextId()%>&wid=<%=cUser.getId() %>" userId="<%=cUser.getId()%>" longName="<%=cUser.getLongName() %>" minPicture="<%=cUser.getMinPicture() %>" profile="<%=cUser.getOrgPicture()%>" userDetail="<%=SmartUtil.getUserDetailInfo(cUser.getUserInfo())%>">
