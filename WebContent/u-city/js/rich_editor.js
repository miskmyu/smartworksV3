...
//		$textarea = $('<div class="form_value" style="width:' + valueWidth + '%"><iframe align="center" frameborder="0" height="100%" width="100%" class="autoHeight" scrolling="no" border="0" onload="richEditorSetValue( $(this), ' + id + ', \'' + smartEncode(value) + '\');"></iframe></div>');
	...
	if(!options.refreshData){
		...
		if (!readOnly) {
			var skinURI = (currentUser.locale == 'ko') ? "u-city/smarteditor/SEditorSkinKOR.html" : "u-city/smarteditor/SEditorSkinENG.html";
...
//			doIframeAutoHeight();
			options.container.find('.form_value span').text('');
			options.container.find('.form_value span').append(isEmpty(value) ? '&nbsp;' : value);
		} else {
...
SmartWorks.FormRuntime.RichEditorBuilder.validate = function(richEditors){
	...
	for(var i=0; i<richEditors.length; i++){
		...
		var value = richEditor.find('textarea')[0] ? richEditor.find('textarea')[0].value : richEditor.find('input')[0].value;
