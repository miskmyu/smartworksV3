...
<input name="hdnFieldType" type="hidden" value="<%=UcityUtil.FIELD_TYPE_COMBO_U_SERVICE %>"/>
...
<span class="str_field">
	<select name="txtFilterStringOperand">
		<%
		String[] serviceNames = Service.getAllServiceNames();
		if(!SmartUtil.isBlankObject(serviceNames)){
			for(int i=0; i<serviceNames.length; i++){
		%>
			<option value="<%=serviceNames[i]%>" <%if(serviceNames[i].equals(operandValue)){%> selected<%} %>><%=serviceNames[i] %></option>
		<%
			}
		}
		%>
